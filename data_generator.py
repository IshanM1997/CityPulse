"""
data_generator.py — Synthetic city taxi & weather data generator
Produces statistically realistic data for any supported city profile.
Real data fetching is attempted first (NYC only); this is the fallback.
"""
...
# ── Hour weights — same pattern for all cities (rush-hour behaviour is universal)
HOUR_WEIGHTS = {
...
def _weighted_list(items: list, weights: list):
    return random.choices(items, weights=weights, k=1)[0]


def get_city_profile(city: str = DEFAULT_CITY) -> dict:
    """Return the profile for a city, falling back to New York."""
    return CITY_PROFILES.get(city, CITY_PROFILES[DEFAULT_CITY])


def generate_trip(city: str = DEFAULT_CITY,
                  base_dt: datetime = None) -> dict:
    """Generate one realistic taxi trip record for the given city."""
    ...
    pickup_hood  = _weighted_choice(profile["neighborhood_weights"])
    dropoff_hood = _weighted_choice(profile["neighborhood_weights"])

    airports    = profile["airports"]
    is_airport  = pickup_hood in airports or dropoff_hood in airports
    ...
    speed_rush  = profile["speed_rush"]
    speed_day   = profile["speed_day"]
    speed_night = profile["speed_night"]

    if hour in range(7, 10) or hour in range(16, 20):
        speed = round(random.uniform(*speed_rush), 1)
    elif hour in range(0, 5):
        speed = round(random.uniform(*speed_night), 1)
    else:
        speed = round(random.uniform(*speed_day), 1)
    ...
    # City-specific fare formula
    base    = profile["base_fare"]
    per_mi  = profile["per_mile_rate"]
    per_min = profile["per_min_rate"]
    fare    = base + (distance * per_mi) + (duration_min * per_min)
    fare    = round(min(fare + random.uniform(-1, 3) * (per_mi / 2.5), MAX_FARE_AMOUNT), 2)
    fare    = max(fare, base)

    payment = _weighted_list(PAYMENT_TYPES, PAYMENT_WEIGHTS)
    tip_pct = 0.0
    if payment == "Credit Card":
        tip_pct = random.choice([0.10, 0.12, 0.15, 0.18, 0.20, 0.25])
        if is_airport:
            tip_pct += 0.05
    tip   = round(fare * tip_pct, 2)
    total = round(fare + tip + random.uniform(0.5, 2.5) * (per_mi / 2.5), 2)
...
def generate_batch(n: int = TAXI_ROWS_PER_RUN,
    ...
    """Generate n trips for a city spread across span_days days."""
    if start_dt is None:
    ...
    for _ in range(n):
        ...
        trips.append(generate_trip(city=city, base_dt=base_dt))
    return trips


def generate_weather_reading(city: str = DEFAULT_CITY,
                              dt: datetime = None) -> dict:
    """Generate one realistic weather reading for any city."""
    if dt is None:
        dt = datetime.now()

    profile    = get_city_profile(city)
    month      = dt.month
    base_temp  = profile["seasonal_temps"].get(month, 65)
...
# ── Real data fetchers (NYC only, with fallback) ───────────────────────────────
...
def fetch_real_weather(city: str = DEFAULT_CITY) -> dict | None:
