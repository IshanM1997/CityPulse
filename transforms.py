...
E → Extract (raw)
T → Transform (clean, validate, enrich, aggregate)
L → Load (into SQLite layers)
"""
...
# Fix the import
AIRPORT_HOODS = {"JFK Airport", "LaGuardia Airport"}
...
def run_dq_checks(df: pd.DataFrame, run_id: str, table: str) -> pd.DataFrame:
    """Apply data quality rules; log results; return cleaned df."""
    ...
    for col, n in nulls.items():
        ...
        log_dq_check(run_id, f"null_check_{col}", table, status, 0, int(n))
        if n > 0:
            issues.append(f"{n} nulls in {col}")
    ...
    # 3. Date sanity — must be within last 5 years
    if "pickup_datetime" in df.columns:
        try:
            ...
            log_dq_check(run_id, "date_range", table,
                         "PASS" if bad_dates==0 else "WARN", "2019-2024", f"{bad_dates} bad")
            df = df[valid_dt]
        except Exception:
            pass

    # 4. Zero-fare trips (suspicious unless no-charge)
    ...
    # 5. Completeness rate
    completeness = round(len(df) / original_count * 100, 1)
...
def extract_trips(raw_trips: list[dict]) -> pd.DataFrame:
    """Convert raw trip dicts to a DataFrame with basic typing."""
    df = pd.DataFrame(raw_trips)
    if df.empty:
        return df

    # Coerce types
    for col in ["fare_amount","tip_amount","total_amount","trip_distance_miles",
...
# ── TRANSFORM ──────────────────────────────────────────────────────────────────
def transform_trips(df: pd.DataFrame, run_id: str) -> pd.DataFrame:
    """
    Full transformation pipeline:
    1. Parse datetimes → extract temporal features
    2. Clean & validate
    3. Enrich (speed, surge flag, airport flag, etc.)
    4. Data quality checks
    """
    if df.empty:
        return df

    print(f"[Transform] Processing {len(df)} raw trip records …")
    ...
    df["is_airport_trip"]= (
        df["pickup_neighborhood"].isin(AIRPORT_HOODS) |
        df["dropoff_neighborhood"].isin(AIRPORT_HOODS)
    ).astype(int)
    ...
    # ── DQ checks (filter bad rows)
    df = run_dq_checks(df, run_id, "stg_taxi_trips")

    # ── Final column selection
    keep_cols = [
        "trip_id","pickup_datetime","dropoff_datetime","pickup_hour",
...
# ── AGGREGATE ──────────────────────────────────────────────────────────────────
def compute_pulse_score(trip_count: float, avg_fare: float,
                        avg_speed: float, surge_pct: float,
                        baseline_trips: float = 500) -> float:
    """
    Composite 'city alive' score (0–100).
    High score = lots of trips, decent speed, high surge usage.
    """
    ...
    raw         = trip_score + fare_score + speed_score + surge_score
    return round(min(raw, 100), 2)


def aggregate_hourly_pulse(df: pd.DataFrame) -> pd.DataFrame:
    """Compute hourly pulse metrics."""
    if df.empty:
        return pd.DataFrame()

    print("[Agg] Computing hourly pulse …")
    ...
    agg["pulse_score"] = agg.apply(lambda r: compute_pulse_score(
        r["trip_count"], r["avg_fare"],
        r["avg_speed_mph"], r["surge_trip_pct"]/100
    ), axis=1)
...
def aggregate_neighborhood_stats(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
    print("[Agg] Computing neighborhood stats …")
...
def aggregate_daily_summary(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame()
    print("[Agg] Computing daily summary …")
    ...
    # Busiest and quietest hour per day
...
# ── LOAD ───────────────────────────────────────────────────────────────────────
def load_raw(trips: list[dict], weather: list[dict]) -> int:
    """Insert raw records to raw layer."""
    now = datetime.utcnow().isoformat()
    with get_conn() as conn:
        for t in trips:
            conn.execute(
                "INSERT INTO raw_taxi_trips (ingested_at, source, raw_json) VALUES (?,?,?)",
                (now, "nyc_tlc", json.dumps(t))
            )
        for w in weather:
            conn.execute(
                "INSERT INTO raw_weather (ingested_at, city, raw_json) VALUES (?,?,?)",
                (now, w.get("city","NYC"), json.dumps(w))
            )
    return len(trips) + len(weather)


def load_staging_trips(df: pd.DataFrame) -> int:
    """Upsert cleaned trips into stg_taxi_trips."""
    if df.empty:
        return 0
    df["loaded_at"] = datetime.utcnow().isoformat()
    with get_conn() as conn:
        df.to_sql("stg_taxi_trips", conn, if_exists="append", index=False,
...
def load_staging_weather(df: pd.DataFrame) -> int:
    if df.empty:
        return 0
    with get_conn() as conn:
        df.to_sql("stg_weather", conn, if_exists="append", index=False,
        ...
        for _, row in df.iterrows():
            conn.execute("""
                INSERT INTO agg_hourly_pulse
                  (date,hour,day_of_week,day_name,trip_count,total_passengers,
            ...
            """, (
                row["date"], int(row["hour"]), int(row["day_of_week"]),
                row["day_name"], int(row["trip_count"]), int(row["total_passengers"]),
        ...
        for _, row in df.iterrows():
            conn.execute("""
                INSERT INTO agg_neighborhood_stats
                  (date,neighborhood,trip_count,avg_fare,total_revenue,avg_distance,rank_by_trips,computed_at)
                VALUES (?,?,?,?,?,?,?,?)
                ON CONFLICT(date,neighborhood) DO UPDATE SET
                  trip_count=trip_count+excluded.trip_count,
                  total_revenue=total_revenue+excluded.total_revenue,
                  avg_fare=(avg_fare+excluded.avg_fare)/2,
                  computed_at=excluded.computed_at
            """, (row["date"], row["neighborhood"], int(row["trip_count"]),
                  row["avg_fare"], row["total_revenue"], row["avg_distance"],
                  int(row["rank_by_trips"]), row["computed_at"]))
    return len(df)
        ...
        for _, row in df.iterrows():
            conn.execute("""
                INSERT INTO agg_daily_summary
                  (date,total_trips,total_passengers,total_revenue,avg_fare,
                ...
                ON CONFLICT(date) DO UPDATE SET
                  ...
                  total_revenue=total_revenue+excluded.total_revenue,
                  computed_at=excluded.computed_at
            """, (row["date"], int(row["total_trips"]), int(row["total_passengers"]),
...
def run_etl(raw_trips: list[dict], raw_weather: list[dict],
    ...
    """
    Orchestrate the full E→T→L pipeline.
    ...
    stats = {
        "run_id": run_id, "raw_trips": len(raw_trips),
        "raw_weather": len(raw_weather),
    ...
    try:
        # 1. Load raw
        load_raw(raw_trips, raw_weather)
        ...
        # 3. Transform
        trips_clean   = transform_trips(trips_df, run_id)
        ...
        # 5. Aggregate
        hourly_df = aggregate_hourly_pulse(trips_clean)
        nbhd_df   = aggregate_neighborhood_stats(trips_clean)
        daily_df  = aggregate_daily_summary(trips_clean)
